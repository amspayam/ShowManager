...
) : AsyncSuspendUseCase<AddMovieRequestModel, CreateMovieMutation.Movie?> {

    override suspend fun executeAsync(rq: AddMovieRequestModel): CreateMovieMutation.Movie? {
        return repository.addNewShow(rq)?.createMovie()?.movie()
    }
}